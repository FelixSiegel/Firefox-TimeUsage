...
let settings_boxes = document.querySelectorAll('.box-title');
settings_boxes.forEach(box => {
    box.addEventListener('click', (e) => {
        e.target.classList.toggle('box-active');
        let content = e.target.parentElement.children[1];
...
document.getElementById("add_blacklist_value_btn").onclick = (e) => {
    let inp = document.getElementById("add_blacklist_value_inp");
    if (inp.value == "") { return }
    ...
    browser.runtime.sendMessage({cmd: "ignore_entry", url: host}).then(
        (response) => {
            ...
            if (response.state != "successful") {
                ...
                let xpath = `//div/div/p[text()='${host}' and contains(@class, 'hostname')]`
                let list_entry = document.evaluate(xpath, document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
...
function add_ignore_entry(host) {
    ...
    item_content.oninput = async (e) => {
        ...
        let changes_wrapper = document.createElement("div");
        ...
        let dismiss_changes = document.createElement("div");
        ...
        let save_changes = document.createElement("div");
        save_changes.classList.add("changes-btn");
        save_changes.innerText = "Save";
        save_changes.onclick = async () => {
            let ignored = (await storageArea.get('ignored'))?.ignored;
            if (!ignored) { return };
            inp_el.innerText = inp_el.innerText.trim()
            let idx = ignored.indexOf(entry.dataset.value);
            if (idx !== -1) {
            ...
            let xpath = `//div/div/p[text()='${inp_el.innerText}' and contains(@class, 'hostname')]`
            let list_entry = document.evaluate(xpath, document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
            if (list_entry) { deleteEntry(list_entry.parentElement.parentElement) }
...
async function loadSettings() {
    ...
    let focus_detection = settings?.focusDetection;
    ...
    // load ignore list
    let ignore_list = (await storageArea.get('ignored'))?.ignored;
